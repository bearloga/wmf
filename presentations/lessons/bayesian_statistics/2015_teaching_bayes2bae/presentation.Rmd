...
fluidPage(
  fluidRow(
    column(numericInput('seed', 'Seed', 0, 1e6, step = 1, value = 0), width = 3),
    column(numericInput('n', 'Trials', 10, 1000, step = 10, value = 30), width = 3),
    column(sliderInput('a', 'α', 0, 100, step = 1, value = 1), width = 3),
    column(sliderInput('b', 'β', 0, 100, step = 1, value = 1), width = 3))
)
renderPlot({
  set.seed(input$seed)
  n <- input$n
  ...
  legend("topright", c(sprintf("Prior: Beta(%.0f,%.0f)", a_prior, b_prior),
                      sprintf("Likelhood & sample prop based on y = %.0f, n = %.0f", y, n),
                      sprintf("Posterior: Beta(%.0f,%.0f)", a_post, b_post),
                      sprintf('"True" success rate: %.2f', p)),
         lty = c("dotted", "dashed", "solid", "solid"), lwd = 2,
         col = c("black", "red", "black", "blue"), bty = "n")
}, width = 800)
```
