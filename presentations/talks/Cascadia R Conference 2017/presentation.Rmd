...
```{r wdqs_advanced_processing, echo=knitr::opts_knit$get("rmarkdown.pandoc.to") != "beamer"}
r_versions_results %<>%
...
kable(rbind(
  head(mutate(r_versions_results, publicationDate = format(publicationDate)), 3),
  data_frame(softwareVersion = "...", publicationDate = "..."),
  tail(mutate(r_versions_results, publicationDate = format(publicationDate)), 3)
), row.names = FALSE)
```
